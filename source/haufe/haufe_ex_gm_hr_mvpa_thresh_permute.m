...
if(proj.flag.clean_build)
    disp(['Removing ',proj.path.haufe.hr_permute_thresh]);
    eval(['! rm -rf ',proj.path.haufe.hr_permute_thresh]);
    disp(['Creating ',proj.path.haufe.hr_permute_thresh]);
    eval(['! mkdir ',proj.path.haufe.hr_permute_thresh]);
end
...
load([proj.path.physio.hr_bpm,'best_thresh.mat']);
best_thresh
...
for i = 1:numel(subjs)
    ...
    if(qlty.ok)
        ...
        all_hr_bpm = [all_hr_bpm;all_bpm(v_ids)];
...
for j = 1:(Nboot+1)
    ...
    save([proj.path.haufe.hr_permute_thresh,'grp_haufe_hr_n', ...
    ...
    if(j>1)
        eval(['! rm ',proj.path.haufe.hr_permute_thresh,'grp_haufe_hr_n', ...
...
save_nii(mu_hr_haufe_nii,[proj.path.haufe.hr_permute_thresh,'mu_hr_haufe_n',num2str(Nboot),'.nii']);
...
save_nii(mu_boot_hr_haufe_nii,[proj.path.haufe.hr_permute_thresh,'mu_boot_hr_haufe_n',num2str(Nboot),'_05.nii']);
...
save_nii(mu_boot_hr_haufe_nii,[proj.path.haufe.hr_permute_thresh,'mu_boot_hr_haufe_n',num2str(Nboot),'_01.nii']);
...
save_nii(mu_boot_hr_haufe_nii,[proj.path.haufe.hr_permute_thresh,'mu_boot_hr_haufe_n',num2str(Nboot),'_001.nii']);