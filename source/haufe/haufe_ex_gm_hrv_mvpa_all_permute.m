...
if(proj.flag.clean_build)
    disp(['Removing ',proj.path.hrv_haufe_permute_all]);
    eval(['! rm -rf ',proj.path.hrv_haufe_permute_all]);
    disp(['Creating ',proj.path.hrv_haufe_permute_all]);
    eval(['! mkdir ',proj.path.hrv_haufe_permute_all]);
end
...
for j = 1:(Nboot+1)
    ...
    save([proj.path.hrv_haufe_permute_all,'grp_haufe_hrv_n', ...
          num2str(Nboot),'_j',num2str(j),'.mat'],'grp_haufe_hrv');

    %% delete the old file that is no longer relevant
    if(j>1)
        eval(['! rm ',proj.path.hrv_haufe_permute_all,'grp_haufe_hrv_n', ...
...
save_nii(mu_hrv_haufe_nii,[proj.path.hrv_haufe_permute_all,'mu_hrv_haufe_n',num2str(Nboot),'.nii']);
...
save_nii(mu_boot_hrv_haufe_nii,[proj.path.hrv_haufe_permute_all,'mu_boot_hrv_haufe_n',num2str(Nboot),'_05.nii']);
...
save_nii(mu_boot_hrv_haufe_nii,[proj.path.hrv_haufe_permute_all,'mu_boot_hrv_haufe_n',num2str(Nboot),'_01.nii']);
...
save_nii(mu_boot_hrv_haufe_nii,[proj.path.hrv_haufe_permute_all,'mu_boot_hrv_haufe_n',num2str(Nboot),'_001.nii']);
